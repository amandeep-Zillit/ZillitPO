//
//  LegacyPOViewModel+cashExpenses.swift
//  ZillitPO
//
...
extension LegacyPOViewModel {
    ...
    /// Fetches the audit trail for a claim batch.
    /// GET /api/v2/cash-expenses/claims/{batchId}/history
    func loadClaimHistory(_ batchId: String, completion: @escaping ([FloatHistoryEntry]) -> Void) {
        claimHistoryLoading = true
        CashExpenseCodableTask.fetchClaimHistory(batchId) { [weak self] result in
            DispatchQueue.main.async {
                self?.claimHistoryLoading = false
                switch result {
                case .success(let r):
                    let entries = r?.data ?? []
    ...
    /// Fetches the query thread raised against a claim batch.
    /// GET /api/v2/account-hub/queries/entity/cash_claim/{batchId}
    /// Entity type is `cash_claim` to match the convention used by the web
    /// client (client/src/components/cash-expenses — PCApprovalPage,
    /// PCMyClaimsPage, etc. all pass `entityType="cash_claim"`). Using
    /// `"claim_batch"` returned an empty thread because no rows on the server
    /// are stored with that entity type.
    func loadClaimQueries(_ batchId: String) {
        claimQueriesLoading = true
        CashExpenseCodableTask.fetchEntityQueries("cash_claim", batchId) { [weak self] result in
            DispatchQueue.main.async {
                self?.claimQueriesLoading = false
                switch result {
    ...
    /// Approve or reject a claim batch. Mirrors the React reference's
    /// `batchApproval(batchId, { action, claim_ids, tier_number, total_tiers, reason })`.
    /// Passing `claimIds: nil` (or equal to the full batch) acts on the entire
    /// batch; a partial set of claim IDs extracts only those claims into a
    /// rejected sub-batch (or approves them at the current tier).
    func approveBatch(id: String, tierNumber: Int, totalTiers: Int,
                      claimIds: [String]? = nil,
        ...
        if let ids = claimIds, !ids.isEmpty { body["claim_ids"] = ids }
        CashExpenseCodableTask.batchApproval(id, body) { [weak self] result in
            DispatchQueue.main.async {
                switch result {
                case .success:
    ...
    func rejectBatch(id: String, reason: String,
                     claimIds: [String]? = nil,
                     completion: @escaping (Bool) -> Void) {
    ...
    /// Override a batch entirely — accountant-only. Requires `can_override`
    /// on the user's cashMeta and `override_receipt_batch` in project settings.
    func overrideBatch(id: String, completion: @escaping (Bool) -> Void) {
        CashExpenseCodableTask.overrideBatch(id) { [weak self] result in
            DispatchQueue.main.async {
                switch result {
                case .success:
    ...
    /// Loads the user's receipt batches. Pass `floatRequestId` to have the
    /// server pre-filter to batches submitted against that specific float
    /// (`/claims/my-batches?float_request_id=<uuid>`). Leave nil for the
    /// unfiltered list — matches the web client's behaviour.
    func loadMyBatches(floatRequestId: String? = nil) {
        isLoadingMyBatches = true
        CashExpenseCodableTask.fetchMyBatches(floatRequestId) { [weak self] result in
            DispatchQueue.main.async {
