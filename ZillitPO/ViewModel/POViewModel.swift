...
class POViewModel: ObservableObject {
    ...
    /// Audit trail + query thread for purchase orders. Mirrors the invoice
    /// pair above — keyed by PO id.
    ...
    /// Audit trail + query thread for claim batches (Receipts History items).
    /// Keyed by batch id.
    ...
    /// Separate flag for Save as Draft / Save as Template / Update
    /// Template paths so the Save dropdown can show its own loader
    /// independently of `formSubmitting` (which drives the Submit
    /// button). Previously both shared `formSubmitting`, so tapping
    /// Submit also spun the Save button.
