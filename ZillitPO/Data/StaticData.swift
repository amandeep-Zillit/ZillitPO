...
struct UsersData {
    ...
    static let byId: [String: AppUser] = Dictionary(uniqueKeysWithValues: allUsers.map { ($0.id, $0) })
}
...
struct DepartmentsData {
    static let all: [Department] = {
        ...
        return UsersData.allUsers.compactMap { u -> Department? in
            guard !seen.contains(u.departmentId) else { return nil }
            seen.insert(u.departmentId)
            return Department(id: u.departmentId, projectId: ProjectData.projectId,
