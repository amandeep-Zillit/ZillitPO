                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if let creator = UsersData.byId[po.userId], !creator.displayDesignation.isEmpty {
                            Divider().padding(.vertical, 4)
                            DetailRow(label: "Designation", value: creator.displayDesignation)
                        }
                        Divider().padding(.vertical, 4)
                        HStack {
                    ...
                    if !po.approvals.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            ForEach(po.approvals.sorted(by: { $0.tierNumber < $1.tierNumber }), id: \.userId) { approval in
                                HStack(spacing: 8) {
                                    ...
                                    VStack(alignment: .trailing, spacing: 2) {
                                        ...
                                        if approval.approvedAt > 0 {
                                            Text(FormatUtils.formatTimestamp(approval.approvedAt))
