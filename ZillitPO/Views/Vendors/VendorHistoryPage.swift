...
// ═══════════════════════════════════════════════════════════════════
// MARK: - Vendor History Page
// Mirrors `POHistoryPage` — a vertical timeline of audit events for
// a single vendor. Icons + colours are derived from the action string
// so the list reads at a glance.
// ═══════════════════════════════════════════════════════════════════

struct VendorHistoryPage: View {
    @EnvironmentObject var appState: POViewModel
    let vendorId: String
    let vendorLabel: String

    private var entries: [InvoiceHistoryEntry] { appState.vendorHistory[vendorId] ?? [] }

    var body: some View {
        ZStack {
            Color.bgBase.edgesIgnoringSafeArea(.all)
            if appState.vendorHistoryLoading && entries.isEmpty {
                ...
                VStack(spacing: 12) {
                    ...
                    Text("Changes to this vendor will appear here.")
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        // Summary header
                        if !vendorLabel.isEmpty {
                            HStack(spacing: 6) {
                                Image(systemName: "person.2.fill").font(.system(size: 12)).foregroundColor(.goldDark)
                                Text(vendorLabel)
        ...
        .navigationBarTitle(Text("Vendor History"), displayMode: .inline)
        .onAppear { appState.loadVendorHistory(vendorId) }
    }

    // MARK: - Styling helpers (matches POHistoryPage)

    private func actionColor(_ action: String) -> Color {
        let a = action.lowercased()
        if a.contains("verif") { return .blue }
        if a.contains("approv") { return .green }
        if a.contains("reject") || a.contains("delet") { return .red }
        if a.contains("updat") || a.contains("edit") { return .goldDark }
        if a.contains("creat") || a.contains("add") { return Color(red: 0.1, green: 0.6, blue: 0.3) }
        return .goldDark
    }

    private func actionIcon(_ action: String) -> String {
        let a = action.lowercased()
        if a.contains("verif") { return "checkmark.seal.fill" }
        if a.contains("approv") { return "checkmark.circle.fill" }
        if a.contains("reject") { return "xmark.circle.fill" }
        if a.contains("delet") { return "trash.fill" }
        if a.contains("updat") || a.contains("edit") { return "pencil.circle.fill" }
        if a.contains("creat") || a.contains("add") { return "plus.circle.fill" }
        return "circle.fill"
    }
    ...
    /// name and (optionally) a designation.
